...
class QuantLinear(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "qweight",
            torch.zeros(
                (math.ceil(infeatures / 32), outfeatures * 32), dtype=torch.int32
            ),
        )
        self.register_buffer(
            "qzeros",
            torch.zeros(
                (math.ceil(infeatures / self.groupsize), outfeatures * 32),
                dtype=torch.int32,
            ),
        )
        self.register_buffer(
            "scales",
            torch.zeros(
                (math.ceil(infeatures / self.groupsize), outfeatures), dtype=dtype
            ),
        )
        self.register_buffer(
            "g_idx",
